...
module.exports = function(settings) {
	...
	function loadSteam(list) {
		...
		return new Promise(function(r, rj) {
			steamAPIKey = settings.server_config.steam_key;
...
//	locstatecode: '11' },
	...
	function loadLog() {
		...
		return new Promise(function(r, rj) {
			...
			readFilePromisified(path.join(path.normalize(settings.server_config.ark_path), "ShooterGame", "Saved", "Logs", "ShooterGame.log"), "utf-8")
			.catch((err) => {
	...
	function setupPlayerFiles() {
		return new Promise(function(resolve, reject) {
			...
			var banData;
			var adminData;
			readFilePromisified(path.join(path.normalize(settings.server_config.ark_path), "ShooterGame", "Binaries", "Linux", "BanList.txt"), "utf-8")
			.then((data) => {
				banData = data;
			})
			.catch((err) => {
			...
			.then(() => readFilePromisified(path.join(path.normalize(settings.server_config.ark_path), "ShooterGame", "Binaries", "Win64", "BanList.txt"), "utf-8"))
			.then((data) => {
				banData = data;
			}).catch((err) => {
			...
			.then(() => readFilePromisified(path.join(path.normalize(settings.server_config.ark_path), "ShooterGame", "Saved", "AllowedCheaterSteamIDs.txt"), "utf-8"))
			.catch((err) => {
			...
			.then(() => readDirPromisified(path.join(path.normalize(path.normalize(settings.server_config.ark_path)), "ShooterGame", "Saved", "SavedArks"), "utf-8"))
			.then((files) => {
				...
				let reqs = files.map((v) => {
					...
					if (re.test(v)) {
						...
						var fdata = fs.statSync(path.join(path.normalize(settings.server_config.ark_path), "ShooterGame", "Saved", "SavedArks", v));
