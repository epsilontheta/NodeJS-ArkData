...
module.exports = function(settings) {
						...
						if (item.Id !== false) {
							if (d === undefined) {
								db.parallelize(function() {
									db.run("INSERT INTO players (id,steamid,charactername,level,engrams,tribeid,banned,fileupdated,filecreated,onlinetimehours,lastonlinetimeupdate) VALUES (?,?,?,?,?,?,?,?,?,?,?)", [item.Id, item.SteamId, item.CharacterName, item.Level, item.TotalEngramPoints, item.TribeId, item.Banned, item.FileUpdated, item.FileCreated, 0, "1900-01-01"], function(err, sql) {
										if (err) {
											db.run("Delete from players where steamid = ?", item.SteamId);
											console.log("LINE 55:", err, "\n Will attempt to fix broken cache record...");
											db.run("INSERT INTO players (id,steamid,charactername,level,engrams,tribeid,fileupdated,banned,filecreated,onlinetimehours,lastonlinetimeupdate) VALUES (?,?,?,?,?,?,?,?,?,?,?)", [item.Id, item.SteamId, item.CharacterName, item.Level, item.TotalEngramPoints, item.TribeId, item.Banned, item.FileUpdated, item.FileCreated, 0, "1900-01-01"], function(err, sql) {
	...
	function loadSteam(list) {
		...
		return new Promise(function(r, rj) {
			steamAPIKey = settings.server_settings.steam_key;
...
//	locstatecode: '11' },
	...
	function loadLog() {
		...
		return new Promise(function(r, rj) {
			...
			readFilePromisified(path.join(path.normalize(settings.server_settings.ark_path), "ShooterGame", "Saved", "Logs", "ShooterGame.log"), "utf-8")
			.catch((err) => {
	...
	function setupPlayerFiles() {
		return new Promise(function(resolve, reject) {
			...
			var banData;
			var adminData;
			readFilePromisified(path.join(path.normalize(settings.server_settings.ark_path), "ShooterGame", "Binaries", "Linux", "BanList.txt"), "utf-8")
			.then((data) => {
				banData = data;
			})
			.catch((err) => {
			...
			.then(() => readFilePromisified(path.join(path.normalize(settings.server_settings.ark_path), "ShooterGame", "Binaries", "Win64", "BanList.txt"), "utf-8"))
			.then((data) => {
				banData = data;
			}).catch((err) => {
			...
			.then(() => readFilePromisified(path.join(path.normalize(settings.server_settings.ark_path), "ShooterGame", "Saved", "AllowedCheaterSteamIDs.txt"), "utf-8"))
			.catch((err) => {
			...
			.then(() => readDirPromisified(path.join(path.normalize(path.normalize(settings.server_settings.ark_path)), "ShooterGame", "Saved", "SavedArks"), "utf-8"))
			.then((files) => {
				...
				let reqs = files.map((v) => {
					...
					if (re.test(v)) {
						var data = fs.readFileSync(path.join(path.normalize(settings.server_settings.ark_path), "ShooterGame", "Saved", "SavedArks", v));
						playerData = {};
						...
						var fdata = fs.statSync(path.join(path.normalize(settings.server_settings.ark_path), "ShooterGame", "Saved", "SavedArks", v));
