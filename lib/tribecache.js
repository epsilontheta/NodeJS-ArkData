...
module.exports = function(settings) {
	...
	function saveTribeLogs(data) {
		...
		return new Promise((r, rj) => {
			...
			let reqs = data.map((item) => {
				return new Promise((resolve) => {
					//purgeOldTribeLogs(item.Id, item.FileUpdated)
					purgeTribeLogs()
					.then(initTable)
					.then(() => {
						if (item.Id !== false) {
	...
	module.setupTribes = function() {
		return new Promise((r, rj) => {
			initTable()
			.then(() => readDirPromisified(path.join(path.normalize(settings.server_settings.ark_path), "ShooterGame", "Saved", "SavedArks"), "utf-8"))
			.then((files) => {
				var players = [];
				var tribeData = {};
				qrylist = [];
				let reqs = files.map((v) => {
					return new Promise(function(resolve) {
						...
						if (re.test(v)) {
							...
							var fdata = fs.statSync(path.join(settings.server_settings.ark_path, "ShooterGame", "Saved", "SavedArks", v));
